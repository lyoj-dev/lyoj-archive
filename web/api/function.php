...
class Application {
    ...
    static function SetDefaultFooter(string $path,array $param):void {
        ...
        for ($i=0;$i<count($config["web"]["footer"]);$i++) {
            ...
            foreach($config["web"]["footer"][$i]["url"] as $key=>$value) {
                $now_content.=InsertTags("a",array("href"=>$value),$key).InsertSingleTag("br",null);
    ...
    static function Output():void {
        ...
        echo InsertTags("html",null,self::$body);
    }
}
...
class Error_Controller {
     ...
     * @return string
     */
    static function Common(string $err):string {
        return InsertTags("script",null,"window.location.href='".
