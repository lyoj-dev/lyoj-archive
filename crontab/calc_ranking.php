...
for ($i=0;$i<count($contest);$i++) {
    $problem=$db->Query("SELECT * FROM problem WHERE contest=".$contest[$i]["id"]);
    ...
    for ($j=0;$j<count($user);$j++) {
        ...
        } $info=array(); for ($k=0;$k<count($problem);$k++) $info[]=array("score"=>0,"time"=>0);
        ...
        for ($k=0;$k<count($problem);$k++) {
            $p=$problem[$k]; $status=$db->Query("SELECT * FROM status WHERE uid=".$u["uid"]." AND pid=".$p["id"]." AND contest=".$c["id"]." ORDER BY id DESC");
            // echo $i." ".$j."SELECT * FROM status WHERE uid=".$u["uid"]." AND pid=".$p["id"]." AND contest=".$c["id"]." ORDER BY id DESC\n";
            ...
            $sumtime+=$sinfo[$k]["time"];
            $sumscore+=$sinfo[$k]["score"];
        } if ($c["type"]!=0||$c["starttime"]+$c["duration"]<time()) 
        $sumtime+=count($db->Query("SELECT id FROM status WHERE contest=".$c["id"]." AND uid=".$u["uid"]." AND status!='Accepted'"))*20*60;
        // echo "$i $j $sumscore $sumtime\n";
