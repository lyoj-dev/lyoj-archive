...
REPO_URL="https://github.com/lyoj-dev/lyoj.git";
EXT_URL="https://github.com/lyoj-dev/lyoj-ext.git";
LANG_URL="https://github.com/lyoj-dev/lyoj-lang.git";
PHP8_URL="https://github.com/php/php-src.git";
ONIG_URL="https://github.com/kkos/oniguruma/releases/download/v6.9.7.1/onig-6.9.7.1.tar.gz";
PROXY_URL="https://ghproxy.com/";
PHP_EXT=( "curl" "date" "hash" "iconv" "imap" "json" "mbstring" "mysqli" "mysqlnd" "openssl" "posix" "sockets" "tidy" );
...
function install_php8() {
    ...
    echo -e "$PURPLE""sudo ""$YELLOW""git ""$BLUE""clone ""$RED""$PHP8_URL""$WHITE";
    sudo git clone $PHP8_URL;
    ...
    echo -e "$PURPLE""sudo ""$YELLOW""./configure ""$GREEN""--prefix=/etc/php/8 --sysconfdir=/etc/php/8 --with-config-file-path=/etc/php/8 --enable-fpm --enable-cgi""$WHITE";
    sudo ./configure --prefix=/etc/php/8 --sysconfdir=/etc/php/8 --with-config-file-path=/etc/php/8 --enable-fpm --enable-cgi;
    if [[ $( echo $? ) != 0 ]]; then 
        echo -e "$RED""PHP 配置失败";
        echo -e "$WHITE""Press \"Enter\" to continue...";
        read -rp "";
    ...
    echo -e "$PURPLE""sudo ""$YELLOW""cp ""$RED""./php.ini-production ""$RED""/etc/php/8/php.ini""$WHITE";
    sudo cp ./php.ini-production /etc/php/8/php.ini;
    if [[ $( echo $? ) != 0 ]]; then 
    ...
    for ext in "${PHP_EXT[@]}"; do
        ...
        echo -e "$YELLOW""cd ""$RED""./php-src/ext/$PHP_EXT""$WHITE";
        cd ./php-src/ext/$PHP_EXT;
        if [[ $( echo $? ) != 0 ]]; then 
        ...
        echo -e "$PURPLE""sudo ""$YELLOW""/etc/php/8/bin/phpize ""$GREEN""--force""$WHITE";
        sudo /etc/php/8/bin/phpize --force
        if [[ $( echo $? ) != 0 ]]; then 
        ...
        echo -e "$PURPLE""sudo ""$YELLOW""./configure ""$GREEN""--with-php-config=/etc/php/8/bin/php-config""$WHITE";
        sudo ./configure --with-php-config=/etc/php/8/bin/php-config;
        echo -e "$PURPLE""sudo ""$YELLOW""make ""$GREEN""-j32 ""$CYAN""&& ""$PURPLE""sudo ""$YELLOW""make ""$BLUE""install""$WHITE";
        sudo make -j32 && sudo make install;
        echo -e "$YELLOW""cd ""$RED""../../../""$WHITE";
        cd ../../../;
    ...
    echo -e "$PURPLE""sudo ""$YELLOW""ln ""$RED""/etc/php/8/bin/php ""$RED""/usr/bin/php8 ""$GREEN""-s""$WHITE";
    sudo ln /etc/php/8/bin/php /usr/bin/php8 -s
    echo -e "$PURPLE""sudo ""$YELLOW""ln ""$RED""/etc/php/8/bin/php-cgi ""$RED""/usr/bin/php-cgi8 ""$GREEN""-s""$WHITE";
    sudo ln /etc/php/8/bin/php-cgi /usr/bin/php-cgi8
    echo -e "$PURPLE""sudo ""$YELLOW""ln ""$RED""/etc/php/8/sbin/php-fpm ""$RED""/usr/bin/php-fpm8 ""$GREEN""-s""$WHITE";
    sudo ln /etc/php/8/sbin/php-fpm /usr/bin/php-fpm8 -s
...
function install() {
    ...
    read -r use;
    if [[ $use == "Y" ]]; then 
        PHP8_URL="$PROXY_URL""$PHP8_URL";
        REPO_URL="$PROXY_URL""$REPO_URL";
        EXT_URL="$PROXY_URL""$EXT_URL";
        ONIG_URL="$PROXY_URL""$ONIG_URL";
    fi 
...
function uninstall() {
    ...
    echo -en "$WHITE""请确认是否需要卸载 LYOJ? [Y/N, Default: N]";
    read -r confirm;
    echo -e "";
    if [[ $confirm != "Y" ]]; then  
    ...
    echo -en "$WHITE""请确认是否需要卸载 PHP8? [Y/N, Default: N]";
    read -r confirm;

    if [[ $confirm == "Y" ]]; then 
        echo -e "$PURPLE""sudo ""$YELLOW""rm ""$RED""/etc/php/8 ""$GREEN""-r""$WHITE";
        sudo rm /etc/php/8 -r;
...
function backup() {
    ...
    echo -e "$YELLOW""提示: 即将将 PHP8 所有数据备份到 ""$SHELL_PATH""php8.zip""$WHITE";

    echo -e "$YELLOW""cd ""$RED""/etc/php/8""$WHITE";
    cd /etc/php/8;
    echo -e "$PURPLE""sudo ""$YELLOW""zip ""$RED""$SHELL_PATH""php8.zip ""$RED""./* ""$GREEN""-r""$WHITE";
    sudo zip "$SHELL_PATH""php8.zip" ./* -r;
    echo -e "$GREEN""PHP8 备份成功";
